...
import type { ILinkifier2, ILinkProvider, IBufferCellPosition, ILink, ILinkifierEvent, ILinkDecorations, ILinkWithState } from 'browser/Types';
...
export class Linkifier2 extends Disposable implements ILinkifier2 {
  ...
  private _positionFromMouseEvent(event: MouseEvent, element: HTMLElement, mouseService: IMouseService): IBufferCellPosition | undefined {
    const offset = this._bufferService.buffer.delegatesScrolling ? this._bufferService.buffer.ybase : 0;
    const coords = mouseService.getCoords(event, element, this._bufferService.cols, this._bufferService.rows + offset);
    if (!coords) {
      return;
    }

    return { x: coords[0], y: coords[1] + this._bufferService.buffer.ybase - offset};
  }
