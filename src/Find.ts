...
export class Find {
  ...
  private _count = document.createElement('span');
  constructor(private _findable: Findable, private _restoreFocus: () => void) {
    this._element.classList.add('find');
    this._count.classList.add('find-count');
    this._element.append(this._input, this._count);
  ...
  _update() {
    ...
    this._findable.setFind?.({
      ...
      report: matches => this._setCount(matches),
    });
  }
  ...
  _setCount(count: number) {
    ...
    this._count.textContent = `${count} matches`;
  }
...
export class FindableList<T extends Findable> implements Findable {
  ...
  setFind(params: FindParams|null) {
    ...
    for (const findable of this._findables)
      findable.setFind(params && {
        ...
        report: matches => {
          ...
          if (this._matchesAtIndex[index] === matches)
            return;
